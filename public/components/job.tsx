...
export const JobElement = ({ k, setActiveMenu, setExpanded, setActivePrint }: { k: Job, setActiveMenu?: Function, setExpanded?: Function, setActivePrint: Function }) => {
    ...
    return (
        <div
            ...
            <div className={`${status == JobStatus.PRINTING ? "bg-green-400" : status == JobStatus.BIDDING ? "bg-orange-400" : status == JobStatus.CANCELED ? "bg-red-400" : status == JobStatus.ENROUTE ? "" : status == JobStatus.PREPRINT ? "bg-yellow-400" : status == JobStatus.COMPLETE ? "bg-green-400" : status == JobStatus.DRAFT ? "bg-orange-400" : status == JobStatus.PREDELIVERY ? "bg-gray-400" : "bg-gray-400"} h-[20px] w-[20px] rounded-full`}></div>
            <div className="">
                <p className="font-bold">{k.job_name}</p>
                <p className="text-gray-600">{status == JobStatus.PRINTING ? `${k.estimated_completion} Remaining` : status == JobStatus.BIDDING ? "Bidding" : status == JobStatus.CANCELED ? "Canceled" : status == JobStatus.DRAFT ? "Draft" : status == JobStatus.ENROUTE ? "En Route" : status == JobStatus.PREPRINT ? "Preparing to print" : "" }</p>
            </div>
        </div>
    )
}